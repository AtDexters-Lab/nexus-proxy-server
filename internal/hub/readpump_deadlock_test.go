...
func TestReadPumpBackpressure_LargePayload(t *testing.T) {
	...
	// Send 45 messages for Client B — enough to trigger the pause at
	// high watermark (40) with 5 more filling headroom. Total 45 < 64
	// (buffer size), so no overflow. The drain goroutine is stuck
	// (net.Pipe, slowRemote not reading), so all 45 stay in the channel.
